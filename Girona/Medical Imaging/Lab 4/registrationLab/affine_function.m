...
switch ttype
    case 'r'   % Make the rigid transformation matrix
        M=[ cos(x(3)) sin(x(3)) x(1);
           -sin(x(3)) cos(x(3)) x(2);
           0 0 1];
    case 'a'  % Make the affine transformation matrix
...
% metric computation
switch mtype
    case 's' %squared differences
        e=sum((I3(:)-Ifixed(:)).^2)/numel(I3);
    case 'cc'
        I_A = sum(sum(Ifixed))/numel(Ifixed);
        I_B = sum(sum(I3))/numel(I3);
        e = -sum((Ifixed(:)-I_A).*(I3(:)-I_B))/sqrt(sum((Ifixed(:)-I_A).^2)*sum((I3(:)-I_B).^2));
