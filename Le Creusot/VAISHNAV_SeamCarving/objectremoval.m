...
clear
tic
image = im2double(imread('./images/9.jpg'));
imshow(image)
...
w = round( rect(3) );
h = round( rect(4) );
% image(ymin:ymin+h,xmin:xmin+w,:)=-100;
% figure, imshow(image)
...
% following paper by Avidan and Shamir '07
    ...
    image = seamCarvingEnlarge([mmax(0, -sizeReductionX), mmax(0, 0)], image);
    image = seamCarvingEnlarge([mmax(0, 0), mmax(0, -sizeReductionY)], image);
end
...
function image = seamCarvingReduce(sizeReduction, image)
    ...
    [~, transBitMask] = findTransportMatrix(sizeReduction, image); % find optimal order of removing rows and columns
...
%    global count
    ...
    %if count ==0
    [i1,i2,i3] = size(I);
        res((ymin):(ymin+h+i1-l),(xmin):(xmin+w+i2-b),:)=-10000;
%         count = count + 1;
    %else
    %end
  %  figure, imshow(res)
